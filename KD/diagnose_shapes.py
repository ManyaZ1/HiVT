...
Usage
-----
    python diagnose_shapes.py \
        --teacher_h5  teacher_outputs/train.h5 \
...
def check_dataset_item(data_root: str, h5_path: str):
    ...
    ok_has_t = check(item.has_teacher is True,              "has_teacher flag",    item.has_teacher)
...
def check_loss_forward(batch_size: int = 4):
    ...
    # Simulate what hivt_kd.py should feed after correct permute
    loc_s   = torch.randn(F, N, H, 2)
    scale_s = torch.rand(F, N, H, 2).abs() + 1e-3
    pi_s    = torch.randn(N, F)

    loc_t   = torch.randn(F, N, H, 2)
    scale_t = torch.rand(F, N, H, 2).abs() + 1e-3
    pi_t    = torch.randn(N, F)

    loss_fn = HiVTKDLoss(lambda_kl=0.5, lambda_pi=0.5)
    try:
        total, metrics = loss_fn(loc_s, scale_s, pi_s, loc_t, scale_t, pi_t)
        ok = check(total.isfinite(), "loss is finite", total.item())
        check(not torch.isnan(total), "loss not NaN", total.item())
