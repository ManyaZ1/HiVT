...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Single-seed runs.} The headline A/B comparisons are single training
        runs. Most effects are far larger than plausible run-to-run noise, but the
        smaller calibration differences would be firmer with multiple seeds (a
        second HiVT-16 seed is available but not yet evaluated).
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Multi-seed variance.} Evaluate the existing second-seed HiVT-16 run
        and add seeds for the HiVT-32 A/Bs, reporting mean$\pm$spread to harden the
        calibration claims (Section~\ref{ssec:res_v2}).
