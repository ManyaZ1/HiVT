...
class HiVTKDLoss(nn.Module):
    ...
    #     # WRONG — shape[1] is N after permute, not F assert pi_s.shape[-1] == loc_s.shape[1] 
    #     # CORRECT — F is always dim-0 of loc, and last dim of pi
    #     assert pi_s.shape[-1] == loc_s.shape[0], (
    #         f"Mode mismatch: pi has {pi_s.shape[-1]} modes but loc has {loc_s.shape[0]} (dim-0)"
    #     )
    ...
    #     #kl_per_mode = kl.mean(dim=(-1, -2))               # [B, F]← comment says [B,F] but gets [F, N]
    #     kl_per_mode = kl.mean(dim=(-1, -2))          # [F, N]
    #     kl_per_mode = kl_per_mode.permute(1, 0)      # [N, F]  == [B, F]
    ...
    #     }
    def forward(self, loc_s, scale_s, pi_s, loc_t, scale_t, pi_t):
        # Expected: loc [F,N,H,2],  pi [N,F]
        # No unsqueeze fallback — callers must pass correctly shaped tensors
        ...
        assert pi_s.shape[-1] == loc_s.shape[0], \
            f"Mode mismatch: pi has {pi_s.shape[-1]} modes but loc has {loc_s.shape[0]} (dim-0=F)"

        teacher_dist = D.Laplace(loc_t, scale_t)
        student_dist = D.Laplace(loc_s, scale_s)

        kl = D.kl_divergence(teacher_dist, student_dist)   # [F, N, H, 2]
        kl_per_mode = kl.mean(dim=(-1, -2))                # [F, N]
        kl_per_mode = kl_per_mode.permute(1, 0)            # [N, F]

        pi_t_soft  = torch.softmax(pi_t, dim=-1)           # [N, F]
        kl_loss    = (pi_t_soft * kl_per_mode).sum(dim=-1).mean()

        log_pi_s   = F.log_softmax(pi_s, dim=-1)           # [N, F]
        pi_ce_loss = -(pi_t_soft * log_pi_s).sum(dim=-1).mean()
