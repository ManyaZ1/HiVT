...
class HiVTKD(pl.LightningModule):
    ...
    def training_step(self, data, batch_idx):
        ...
        if has_teacher:
            ...
            student_loc = pred_s[:, agent_index, :, :2] #student_loc: [F, B, H, 2]
            student_scale = pred_s[:, agent_index, :, 2:] #[F, B, H, 2]
            student_pi = pi_s[agent_index] #[B, F]
            if student_loc.dim() == 3: #Single agent (edge case)
                #Shape: from [F, H, 2] => [1, F, H, 2]
                student_loc = student_loc.unsqueeze(0)
                student_scale = student_scale.unsqueeze(0)
                student_pi = student_pi.unsqueeze(0)
            else:
                # from [F, B, H, 2] => [B, F, H, 2]
                student_loc = student_loc.permute(1, 0, 2, 3).contiguous()
                student_scale = student_scale.permute(1, 0, 2, 3).contiguous()
            # batch.teacher_loc shape after PyG stacks N x [1,6,30,2]: [N,1,6,30,2]
            loc_t   = data.teacher_loc.squeeze(1)                  # [N,1,6,30,2] → [N,6,30,2]
            scale_t = data.teacher_scale.squeeze(1)
            pi_t    = data.teacher_pi.squeeze(1)                   # [N,1,6] → [N,6]

            # now permute to match student convention [F,N,H,2]
            loc_t   = loc_t.permute(1, 0, 2, 3).to(self.device)   # [N,6,30,2] → [6,N,30,2]
            scale_t = scale_t.permute(1, 0, 2, 3).to(self.device)
            pi_t    = pi_t.to(self.device)                         # [N,6] — correct as-is
